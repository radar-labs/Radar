...
extension ConversationSettingsViewController {
    ...
    func updateTableContents(shouldReload: Bool = true) {
        ...
        addStarredMessagesItem(to: mainSection)
        ...
        if let groupModel = currentGroupModel, !groupModel.isPlaceholder {
            contents.add(
                buildGroupMembershipSection(
                    groupModel: groupModel, sectionIndex: contents.sections.count))
        ...
        section.add(
            .init(
                customCellBlock: { [weak self] in
                    ...
                    let availableWidth =
                        self.view.width
                        - ((Self.cellHInnerMargin * 2) + self.cellOuterInsets.totalWidth
                            + self.view.safeAreaInsets.totalWidth)
                    let imageWidth =
                        (availableWidth - totalSpacerSize) / CGFloat(self.maximumRecentMedia)
                    ...
                    if self.recentMedia.count < self.maximumRecentMedia {
                        ...
                        seeAllLabel.autoSetDimension(
                            .height, toSize: OWSTableItem.primaryLabelFont.lineHeight)
    ...
    private func addBadgesItemIfNecessary(to contents: OWSTableContents) {
        ...
        let (visibleBadges, shortName) = SSKEnvironment.shared.databaseStorageRef.read {
            tx -> ([OWSUserProfileBadgeInfo], String) in
            let visibleBadges: [OWSUserProfileBadgeInfo] = {
                let tsAccountManager = DependenciesBridge.shared.tsAccountManager
                guard let localIdentifiers = tsAccountManager.localIdentifiers(tx: tx) else {
                    return []
                }
                let address = OWSUserProfile.internalAddress(
                    for: contactAddress, localIdentifiers: localIdentifiers)
                guard let userProfile = OWSUserProfile.getUserProfile(for: address, tx: tx) else {
                    return []
                }
                return userProfile.visibleBadges
            }()
            let shortName = SSKEnvironment.shared.contactManagerRef.displayName(
                for: contactAddress, tx: tx
        ...
        contents.add(
            .init(
                title: OWSLocalizedString(
                    "CONVERSATION_SETTINGS_BADGES_HEADER",
                    comment: "Header title for a contact's badges in conversation settings"),
                items: [
                    OWSTableItem(
                        customCellBlock: { [weak self] in
                            let cell = OWSTableItem.newCell()
                            ...
                            collectionView.badgeSelectionMode = .detailsSheet(
                ...
                footerTitle: OWSLocalizedString(
                    "CONVERSATION_SETTINGS_BADGES_FOOTER",
                    comment: "Footer string for a contact's badges in conversation settings"))
        )
    }
                ...
                customCellBlock: { [weak self] in
                    ...
                    let cell = OWSTableItem.buildCell(
                        ...
                        accessibilityIdentifier: UIView.accessibilityIdentifier(
                            in: self, name: "color_and_wallpaper")
                    )
                    return cell
                },
                actionBlock: { [weak self] in
            ...
            OWSTableItem(
                customCellBlock: { [weak self] in
                    ...
                    let cell = OWSTableItem.buildCell(
                        ...
                        accessoryType: .disclosureIndicator,
                        accessibilityIdentifier: UIView.accessibilityIdentifier(
                            in: self, name: "sound_and_notifications")
                    )
                    return cell
                },
                actionBlock: { [weak self] in
    ...
    private func addStarredMessagesItem(to section: OWSTableSection) {
        section.add(
            OWSTableItem(
                customCellBlock: { [weak self] in
                    guard let self = self else {
                    ...
                    let cell = OWSTableItem.buildCell(
                        ...
                        accessoryType: .disclosureIndicator,
                        accessibilityIdentifier: UIView.accessibilityIdentifier(
                            in: self, name: "starred_messages")
                    )
                    return cell
                },
                actionBlock: { [weak self] in
                    self?.showStarredMessagesView()
                }))
    }
            ...
            .init(
                customCellBlock: { [weak self] in
                    ...
                    let cell = OWSTableItem.buildCell(
                        ...
                        accessoryType: .disclosureIndicator,
                        customColor: canEditConversationAttributes
                            ? nil : Theme.secondaryTextAndIconColor,
                        accessibilityIdentifier: UIView.accessibilityIdentifier(
                            in: self, name: "disappearing_messages")
                    )
                ...
                actionBlock: { [weak self] in
                    let vc = DisappearingMessagesTimerSettingsViewController(
                        ...
                        NotificationCenter.default.post(
                            name: ChatListViewController.clearSearch, object: nil)
                    }
                    self?.presentFormSheet(
                        OWSNavigationController(rootViewController: vc), animated: true)
                }
            ))
    }
            ...
            .item(
                ...
                actionBlock: { [weak self] in
                    ...
                    let navigationController = OWSNavigationController(
                        rootViewController: nicknameEditor)
    ...
    private func buildBlockAndLeaveSection() -> OWSTableSection {
        ...
        let hasReportedSpam = SSKEnvironment.shared.databaseStorageRef.read { tx in
            return InteractionFinder(threadUniqueId: thread.uniqueId).hasUserReportedSpam(
                transaction: tx)
        }
    ...
    private func buildGroupMembershipSection(groupModel: TSGroupModel, sectionIndex: Int)
        -> OWSTableSection
    {
        let section = OWSTableSection()
        section.separatorInsetLeading =
            Self.cellHInnerMargin + CGFloat(AvatarBuilder.smallAvatarSizePoints)
        ...
        let format = OWSLocalizedString(
            "CONVERSATION_SETTINGS_MEMBERS_SECTION_TITLE_%d", tableName: "PluralAware",
            comment:
                    ...
                    customCellBlock: { [weak self] in
                        ...
                        SSKEnvironment.shared.databaseStorageRef.read { transaction in
                            let configuration = ContactCellConfiguration(
                                address: memberAddress, localUserDisplayMode: .asLocalUser)
                            let isGroupAdmin = groupMembership.isFullMemberAndAdministrator(
                                memberAddress)
                            let isVerified = verificationState == .verified
                            let isNoLongerVerified = verificationState == .noLongerVerified
                            let isBlocked = SSKEnvironment.shared.blockingManagerRef
                                .isAddressBlocked(memberAddress, transaction: transaction)
                            if isGroupAdmin {
                                ...
                                configuration.accessoryMessage =
                                    MessageStrings.conversationIsBlocked
                            }
                            ...
                            if isVerified {
                                ...
                                let bioForDisplay = SSKEnvironment.shared.profileManagerImplRef
                                    .userProfile(for: memberAddress, tx: transaction)?.bioForDisplay
                            {
                                configuration.attributedSubtitle = NSAttributedString(
                                    string: bioForDisplay)
                            } else {
                            ...
                            let isSystemContact =
                                SSKEnvironment.shared.contactManagerRef.fetchSignalAccount(
                                    for: memberAddress, transaction: transaction) != nil
        ...
        if hasMoreMembers {
            ...
            let expandedMemberIndices =
                ((membersToRender.count + offset)..<(totalMemberCount + offset)).map {
    ...
    private func buildGroupSettingsSection(
        ...
        let invitedOrRequestingCount =
            groupModelV2.groupMembership.invitedMembers.count
            + groupModelV2.groupMembership.requestingMembers.count
        section.add(
            OWSTableItem.disclosureItem(
    ...
    private func buildMutualGroupsSection(sectionIndex: Int) -> OWSTableSection {
        ...
        section.add(
            OWSTableItem(
                customCellBlock: { [weak self] in
                    ...
                    let rowLabel = UILabel()
                    rowLabel.text = OWSLocalizedString(
                        "ADD_TO_GROUP_TITLE", comment: "Title of the 'add to group' view.")
                    rowLabel.textColor = Theme.primaryTextColor
                    rowLabel.font = OWSTableItem.primaryLabelFont
                    rowLabel.lineBreakMode = .byTruncatingTail
        ...
        if mutualGroupThreads.count > 0 {
            let headerFormat = OWSLocalizedString(
                "CONVERSATION_SETTINGS_MUTUAL_GROUPS_SECTION_TITLE_%d", tableName: "PluralAware",
                comment:
            ...
            section.headerTitle = String.localizedStringWithFormat(
                headerFormat, mutualGroupThreads.count)
        } else {
            section.headerTitle = OWSLocalizedString(
        ...
        if hasMoreGroups {
            let expandedGroupIndices =
                ((groupThreadsToRender.count + 1)..<(mutualGroupThreads.count + 1)).map {
                    IndexPath(row: $0, section: sectionIndex)
                }
