...
public class InteractionFinder: NSObject {
  ...
  public class func unreadCountInAllThreads(transaction: DBReadTransaction) -> UInt {
    do {
      let includeMutedThreads = SSKPreferences.includeMutedThreadsInBadgeCount(
        transaction: transaction)
      ...
      if !includeMutedThreads {
        unreadInteractionQuery +=
          " \(sqlClauseForIgnoringInteractionsWithMutedThread(threadAssociatedDataAlias: "associatedData")) "
      }

      unreadInteractionQuery +=
        " AND \(sqlClauseForUnreadInteractionCounts(interactionsAlias: "interaction")) "

      let unreadInteractionCount = try UInt.fetchOne(
        transaction.database, sql: unreadInteractionQuery)
      owsAssertDebug(
        unreadInteractionCount != nil, "unreadInteractionCount was unexpectedly nil")
      ...
      if !includeMutedThreads {
        markedUnreadThreadQuery +=
          " \(sqlClauseForIgnoringInteractionsWithMutedThread(threadAssociatedDataAlias: "associatedData")) "
      }

      let markedUnreadCount = try UInt.fetchOne(
        transaction.database, sql: markedUnreadThreadQuery)
  ...
  public class func fetchSomeExpiredMessageRowIds(now: UInt64, limit: Int, tx: DBReadTransaction)
    throws -> [Int64]
  {
    // NOTE: We DO NOT consult storedShouldStartExpireTimer here;
    //       once expiration has begun we want to see it through.
  ...
  public func profileUpdateInteractions(
    ...
    do {
      ...
      owsAssertDebug(
        allResults.allSatisfy({ ($0 as? TSInfoMessage)?.profileChangeAddress != nil }))
    } catch {
      owsFailDebug("Unexpected error \(error)")
      allResults = []
    }
  ...
  func latestInteraction(
    ...
    let sql = """
      ...
      """
    let arguments: StatementArguments = [
      threadUniqueId, address.serviceIdUppercaseString, address.phoneNumber,
    ]
  ...
  public func hasMessagesToMarkRead(
    ...
    let hasOutgoingMessagesWithUnreadReactions =
      (try? Bool.fetchOne(
        transaction.database,
        sql: """
          SELECT 1
  ...
  public class func outgoingAndIncomingMessageCount(transaction: DBReadTransaction, limit: Int)
    -> UInt
  {
    let sql = """
      SELECT COUNT(*)
  ...
  public func fetchUniqueIdsForConversationView(
    ...
    let (rowIdClause, arguments, isAscending) = sqlClauseForInteractionsByRowId(
      rowIdFilter: rowIdFilter,
      additionalFiltering: onlyStarred
        ? .filterForConversationViewWithOnlyStarred : .filterForConversationView,
      limit: limit
    )
  ...
  private enum InteractionsByRowIdAdditionalFiltering {
    ...
    case filterForConversationViewWithOnlyStarred
  }
  ...
  private func buildInteractionCursor(
    ...
    case .filterForConversationView, .filterForConversationViewWithOnlyStarred:
      indexedBy = "INDEXED BY index_interactions_on_threadUniqueId_and_id"
    case .filterForIncomingMessages:
      indexedBy = DEBUG_INDEXED_BY(
        "index_interactions_on_recordType_and_threadUniqueId_and_errorType")
    case .filterForOutgoingMessages:
      indexedBy = DEBUG_INDEXED_BY(
        "index_interactions_on_recordType_and_threadUniqueId_and_errorType")
    case .noFiltering:
  ...
  private func sqlClauseForInteractionsByRowId(
    ...
    case .range(let rowIds):
      rowIdFilterClause =
        "AND \(interactionColumn: .id) >= ? AND \(interactionColumn: .id) <= ?"
      rowIdArguments = [rowIds.lowerBound, rowIds.upperBound]
      isAscending = true
    }

    let additionalFilterClause: String =
      switch additionalFiltering {
      case .filterForConversationView:
        """
        \(Self.filterGroupStoryRepliesClause())
      ...
      case .filterForConversationViewWithOnlyStarred:
        ...
        \(Self.filterEditHistoryClause())
        \(Self.filterPlaceholdersClause)
        AND IsStarred = 1 AND errorType is NULL
        """
      case .filterForIncomingMessages:
...
extension TSInteractionCursor {
  fileprivate func enumerate(block: (TSInteraction) -> Bool) throws {
    while let interaction = try next(),
      block(interaction)
    {}
  }
}
...
extension InteractionFinder {
  private static func sqlClauseForAllUnreadInteractions(
    ...
    let editQueryMode: EditMessageQueryMode =
      excludeReadEdits ? .excludeReadEdits : .includeAllEdits
  ...
  private static func sqlClauseForIgnoringInteractionsWithMutedThread(
    threadAssociatedDataAlias: String
  ) -> String {
    """
    AND (
  ...
  static let filterPlaceholdersClause =
  ...
  static func filterEditHistoryClause(
    ...
    case .includeAllEdits:
      ...
      return
        "AND \(columnPrefix)\(interactionColumn: .editState) IS NOT \(TSEditState.pastRevision.rawValue)"
    case .excludeReadEdits:
      return """
        AND (
    ...
    case .excludeAllEdits:
      return
        "AND \(columnPrefix)\(interactionColumn: .editState) IS \(TSEditState.none.rawValue)"
    }
  }
